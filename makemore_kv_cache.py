...
class Block(nn.Module):
    ...
    def forward(self, x, kvcache=None):
        x = x + self.attn(self.ln_1(x),kvcache)
...
class Transformer(nn.Module):
    ...
    def forward(self, idx, targets=None, kvcache=None):
        ...
        device = idx.device
        if not kvcache:
            kvcache = [None]*len(self.transformer.h)
            start = 0
        else:
            idx = idx[:, -1:]
            start = t-1
        ...
        pos = torch.arange(start, t, dtype=torch.long, device=device).unsqueeze(0) # shape (1, t)
        ...
        x = tok_emb + pos_emb
        new_kvcache = []
        for block, kv in zip(self.transformer.h, kvcache):
            x, kv = block(x, kv)
...
def generate(model, idx, max_new_tokens, temperature=1.0, do_sample=False, top_k=None):
    ...
    for _ in range(max_new_tokens):
        ...
        logits, _ , kvcache = model(idx_cond, kvcache=kvcache)
